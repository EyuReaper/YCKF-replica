...
import { AuditLog } from '../models/AuditLog.js'; // Updated for bots
import NodeCache from 'node-cache'; // For rate limiting
import client from '../lib/sanity.client..js'; // Sanity client
...
export const getFAQMatch = async (query: string, category?: string) => {
  const faqQuery = groq`*[_type == "faq" && (category == $category || !defined($category)) && score(title + " " + answer, $query) > 0.3] | order(score(title + " " + answer, $query) desc) [0...1] {
    _id,
    title,
    answer,
    category,
  ...
  const faq = await client.fetch(faqQuery, params);
...
export async function processBotMessage(query: string, userId: string, channel: 'whatsapp' | 'telegram'): Promise<string> {
  try {
    ...
    await logBotInteraction(userId, channel, query, 'N/A', undefined, 'failed', error.message);
